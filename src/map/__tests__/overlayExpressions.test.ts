/**
 * Overlay 表達式靜態驗證 —— 用 MapLibre style-spec validator 掃 OVERLAY_REGISTRY
 * 每一個 layer 的 paint / layout / filter。
 *
 * 為什麼要有這支（2026-08-02 山域事故實例）：
 *   `["*", ["interpolate",["linear"],["zoom"],…], ["step",…]]`
 *   看起來完全合理、tsc 全綠、測試全綠 —— 但引擎在 addLayer 時**整層拒收**，
 ...
 * layout / filter 的取用順序），確保驗到的就是實際送進 MapLibre 的東西。
 */
import { describe, it, expect } from "vitest";
import { validateStyleMin } from "@maplibre/maplibre-gl-style-spec";
import { OVERLAY_REGISTRY } from "../overlayRegistry";
import { normalizePaintForMapLibre } from "../overlayManager";
...
function buildLayer(
  ...
  return {
    ...
    paint: normalizePaintForMapLibre(spec.type, spec.paint(isDark, params)),
  };
}
...
function validateLayer(layer: Record<string, unknown>, source: Record<string, unknown>): string[] {
  const style = {
    version: 8,
    // symbol layer 的 text-field 需要 style 有 glyphs（真實環境由 Liberty 底圖提供）；
...
describe("overlay 表達式（MapLibre style-spec 靜態驗證）", () => {
